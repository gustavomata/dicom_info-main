...
def calculate_slice_thickness(ds):
    try:
        return "{:.2f}".format(float(getattr(ds, 'SliceThickness', "N/D")))
    except ValueError:
        return "N/A"

def get_sex(ds):
    return getattr(ds, 'PatientSex', "N/D")
...
def generate_pdf_report_and_open(tree):
    ...
    # Remove o cabeçalho da coluna "Pasta" antes de gerar o relatório PDF
    headers = list(table_data[0])
    if "Pasta" in headers:
        headers.remove("Pasta")
    ...
    for col in tree["columns"]:
        if col not in headers:  # Verifica se a coluna está presente nos cabeçalhos desejados
            table_data[0].remove(col)  # Remove o cabeçalho da coluna
...
def show_dicom_info(main_directory):
    ...
    def toggle_dark_mode():
        ...
        if current_theme == "":
            ...
            entry_search = ttk.Entry(frame_buttons, style="DarkMode.TEntry")

        entry_search.grid(row=1, column=0, columnspan=3, pady=5, padx=5, sticky="nsew")  # Atualize a grade do campo de busca
